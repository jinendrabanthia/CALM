            ...
            try:
                ...
                if vol > VOLUME_THRESHOLD:
                    ...
                    if top_label in DISTRESS_LABELS and top_score > CONFIDENCE_THRESHOLD or vol > 0.6:
                        self.log(f"[!!!] INJURY DETECTED: {top_label}", "alert")
                        self.set_status("DISTRESS DETECTED", f"Identified: {top_label}", "#ff0000")
                        ...
                        while in_conversation:
                            ...
                            try:
                                self.set_status("LISTENING", "Waiting for response...", "#00ccff")
                                user_audio = listen_to_existing_stream(stream, self.log)
                                text = r.recognize_google(user_audio)
                                self.log(f"User: {text}", "user")

                                if any(phrase in text.lower() for phrase in ["satisfied", "i am good", "care is complete", "stop","deactivate"]):
                                    speak("Understood. Resuming watch.", self.log)
                                    in_conversation = False
                                    break
